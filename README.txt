...
#########################Discussion###########################
##Reason of Choose Injury and Coach
We wrote a simple word count program to count the frequency of each word in NBA news, and eliminate the meaningless words like ‘as’, ‘the’ and etc. The injury information and the word ‘coach’ have appeared over 100 times, so we choose these two topics to build the knowledge base.

##Future of DeepDive
The value of DeepDive would be it’s open source, and the performance of the KBC system it built can be keep modifying by increasing number of developers. Eventually, the ideal condition DeepDive can contribute is, when somebody have same raw data to process, instead of using DeepDive to build a KBC, one just need to find the specific KBC system developed by DeepDive online (if Stanford will always keep DeepDive open source). 